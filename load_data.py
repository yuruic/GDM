...
def read_ba2motif_data(folder: str, prefix):
    ...
    for graph_idx in range(dense_edges.shape[0]):
        ## adding one hot feature
        onehot = F.one_hot(torch.arange(0, 25) % 25)
        onehot = onehot.float()
        ...
        data_list.append(Data(x=onehot,
                              edge_index=dense_to_sparse(torch.from_numpy(dense_edges[graph_idx]))[0],
                             y=torch.from_numpy(np.where(graph_labels[graph_idx])[0])))
    return data_list
...
def get_dataset(dataset_dir, dataset_name, task=None):
    ...
    elif dataset_name.lower() == 'dblp_ct1'.lower():
        return load_dblp(dataset_dir, 'dblp_ct1')
    elif dataset_name.lower() in sync_dataset_dict.keys():
        sync_dataset_filename = sync_dataset_dict[dataset_name.lower()]
        print(dataset_dir,sync_dataset_filename)
...
def load_dblp(dataset_dir, dataset_name):
    dataset = DBLPDataset(root=dataset_dir, name=dataset_name)
    return dataset
...
class BA_LRP(InMemoryDataset):
    ...
    def gen_class1(self):
        ...
        for i in range(2, 20):
            ...
            data.x = F.one_hot(torch.arange(0, 20) % 20).float()
...
def get_dataloader(dataset, batch_size, random_split_flag=True, data_split_ratio=None, seed=2):
    ...
    return train,train_datalist,dataloader['train'],dataloader['eval'],dataloader['test'] 